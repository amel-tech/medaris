...
Expected: typecheck 16 projects · **299 tests / 22 suites** · build 8 · lint 16 · module-boundaries 16.

Two prerequisites that look optional and are not:

- **`-t test` needs a running Docker daemon.** `apps/tedrisat/vitest.config.ts` matches `test/**/*.spec.ts`, which includes the eight `test/e2e/*.e2e.spec.ts` suites, and those start a Testcontainers `postgres:17-alpine`. Of tedrisat's 20 suites, 8 are e2e. `test:e2e` re-runs the same eight under a separate config — it is not extra coverage.
