...
- **The tedrisat suite may not reach the network (MDRS-89).** `apps/tedrisat/test/setup-no-network.ts` replaces `globalThis.fetch` in every worker and throws on any host that is not loopback. A new dependency that fetches something fails the run rather than logging — which is what `KeycloakPublicKeyProvider` did for months while every `createTestApp` called the live realm. Containers on loopback are fine; that is how `keycloak-audience.e2e.spec.ts` talks to its own Keycloak.
