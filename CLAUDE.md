...
- **`-t build` needs `apps/<app>/.env` for the four Next.js apps.** They validate the environment at build time, so a fresh worktree fails with `Invalid environment variables` until `cp .env.example .env && pnpm env:sync` has written them. That is a missing file, not a regression.
...
- **`apps/*/.env.example` are generated (MDRS-25).** Edit the root `.env.example` and run `pnpm env:sync`; a hand edit is reverted on the next sync and `pnpm env:check` fails CI in the meantime.
