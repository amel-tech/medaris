...
export const KoskListPage = ({
  ...
  const levelLabel = (level: string | null) => {
    switch (level) {
      case "ALL":
        return t("KoskListPage.allLevels");
      case "BEGINNER":
  ...
  const visible = useMemo(() => {
    ...
    const list = kosks.filter((k) => {
      ...
      if (field && (k.field ?? t("KoskListPage.uncategorized")) !== field)
        return false;
      if (
        q &&
        !`${k.name} ${k.handle ?? ""} ${k.description ?? ""}`
      ...
      {continuing.length > 0 && (
        <section className="mb-8">
          <div className="mb-3.5 flex items-center justify-between">
            ...
            >
              {t("KoskListPage.allMyCourses")} <ArrowRight size={13} />
            </Link>
          </div>
      ...
      <div className="grid grid-cols-[200px_1fr] gap-7">
        <aside>
          ...
          <ul className="flex flex-col gap-0.5">
            <FieldItem
              label={t("KoskListPage.allFields")}
              count={kosks.length}
              active={!field}
        ...
        <main>
          <div className="mb-5 flex items-center gap-2">
            ...
            >
              {t("KoskListPage.sortPopular")} <CaretDown size={13} />
            </button>
          </div>
  ...
  >
    ...
    <div className="flex flex-1 flex-col px-4 pb-4 pt-8">
      <div className="flex items-center gap-2">
        ...
        {kosk.verified && (
          <SealCheck size={15} weight="fill" className="text-blue-700" />
        )}
        <span className="flex-1" />
      ...
      {kosk.tags.length > 0 && (
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          {kosk.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="rounded bg-slate-100 px-2 py-0.5 text-[11px] font-medium"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="mt-auto flex items-center justify-between pt-3.5">
        <div className="flex gap-3.5 text-xs text-muted-foreground">
          <span>
            {kosk.courseCount} {t("KoskListPage.coursesShort")}
          </span>
          <span>
            {kosk.studentCount} {t("KoskListPage.studentsShort")}
          </span>
        </div>
        <FollowButton
          kosk={kosk}
