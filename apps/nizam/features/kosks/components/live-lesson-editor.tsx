...
export const LiveLessonEditor = ({
  ...
  const patchStep = (i: number, p: Partial<AgendaStepDraft>) =>
    patch({
      agenda: draft.agenda.map((s, idx) => (idx === i ? { ...s, ...p } : s)),
    });
    ...
    <div className="m-1 rounded-xl border bg-white p-4 shadow-lg">
      ...
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <span className="text-sm font-bold">
          {draft.id
            ? t("NewCoursePage.editLiveLesson")
            : t("NewCoursePage.newLiveLesson")}
        </span>
        <button
          type="button"
          onClick={onCancel}
      ...
      <div className="mt-3.5 grid grid-cols-1 gap-3 sm:grid-cols-[1fr_140px]">
        ...
        <div>
          <Label className="mb-1.5 text-xs font-medium">
            {t("NewCoursePage.liveLessonDuration")}
          </Label>
          <Input
            type="number"
            min={5}
        ...
        {draft.meetingUrl.trim() && (
          <div className="mt-2 flex items-center gap-2">
            ...
            >
              ...
              />
              {platform.id === "unknown"
                ? t("NewCoursePage.platformUnknown")
                : platform.label}
            </span>
      ...
      <div className="mt-4">
        <div className="mb-2 flex items-baseline gap-2">
          <span className="text-xs font-medium">
            {t("NewCoursePage.agendaTitle")}
          </span>
          <span className="text-xs text-muted-foreground">
            {t("NewCoursePage.agendaHint")}
          </span>
        </div>
          ...
          {draft.agenda.map((step, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() =>
                  patch({ agenda: draft.agenda.filter((_, idx) => idx !== i) })
                }
      ...
      <div className="mt-4 flex justify-end gap-2 border-t pt-3.5">
        ...
        >
          <Check size={15} /> {t("NewCoursePage.saveLiveLesson")}
        </button>
      </div>
    </div>
  );
