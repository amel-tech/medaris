...
export const NewCoursePage = ({
  ...
  const [level, setLevel] = useState<CreateCourseDtoLevelEnum>(
    (course?.level as CreateCourseDtoLevelEnum) ??
      CreateCourseDtoLevelEnum.Beginner
  );
  const [grantsCertificate, setGrantsCertificate] = useState(
    course?.grantsCertificate ?? false
  );
    ...
    course?.weeks.length
      ? course.weeks.map((w) => ({
          ...
          lessons: w.lessons.map((l) => ({
            ...
            meetingUrl: l.meetingUrl ?? "",
            agenda:
              l.agenda?.map((s) => ({ time: s.time, title: s.title })) ?? [],
          })),
        }))
      : [newWeek()]
  );
      ...
      weeks.map((w, idx) => {
        ...
        const lessons =
          editor.li === null
            ...
            : w.lessons.map((l, j) =>
                j === editor.li ? fromLiveDraft(editor.draft, l) : l
              );
        return { ...w, lessons };
      })
    );
  ...
  const copyWeek = (wi: number) => {
    ...
    const clone: WeekDraft = {
      title: src.title.trim()
        ? `${src.title} ${t("NewCoursePage.weekCopySuffix")}`
        : "",
      summary: src.summary,
  ...
  const lessonCount = useMemo(
    () =>
      weeks.reduce(
        (s, w) => s + w.lessons.filter((l) => l.title.trim()).length,
        0
      ),
    [weeks]
  );
  ...
  const submit = (status: CreateCourseDtoStatusEnum) => {
    ...
    startTransition(async () => {
      ...
      toast.success(
        t(isEdit ? "NewCoursePage.updated" : "NewCoursePage.created")
      );
    ...
    <div className="mx-auto max-w-6xl pb-16">
      ...
      <div className="mb-6 flex items-end justify-between border-b pb-5">
        ...
        <div className="flex gap-2">
          ...
          >
            {pending
              ? t("NewCoursePage.publishing")
              : t("NewCoursePage.publish")}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-7 lg:grid-cols-[1fr_340px]">
        ...
        <div className="flex flex-col gap-6">
          {/* Basic info */}
          <Section
            title={t("NewCoursePage.sectionBasic")}
            hint={t("NewCoursePage.sectionBasicHint")}
          >
            <Field label={t("NewCoursePage.fieldTitle")} required>
              <Input value={title} onChange={(e) => setTitle(e.target.value)} />
            </Field>
            <Field
              label={t("NewCoursePage.fieldDescription")}
              hint={t("NewCoursePage.fieldDescriptionHint")}
            >
              <Textarea
                rows={3}
                value={description}
            ...
            <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
              ...
              <Field label={t("NewCoursePage.fieldLevel")}>
                <Select
                  value={level}
                  onValueChange={(v) => setLevel(v as CreateCourseDtoLevelEnum)}
                >
          ...
          {/* Teachers & resources */}
          <Section
            title={t("NewCoursePage.sectionTeachers")}
            hint={t("NewCoursePage.sectionTeachersHint")}
          >
            <Field
              label={t("NewCoursePage.muderris")}
              required
              error={
                attempted && muderrisInvalid
                  ? t("NewCoursePage.validationMuderris")
                  : undefined
              }
            >
            ...
            <Field label={t("NewCoursePage.resources")}>
              <div className="flex flex-col gap-2">
                {resources.map((r, i) => (
                  <div key={i} className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        setResources(
                          upd(resources, i, { name: e.target.value })
                        )
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setResources(
                          upd(resources, i, { meta: e.target.value })
                        )
                      }
                    />
                    <button
                ...
                <AddButton
                  onClick={() =>
                    setResources([...resources, { name: "", meta: "" }])
                  }
                >
          ...
          {/* Curriculum */}
          <Section
            title={t("NewCoursePage.sectionCurriculum")}
            hint={t("NewCoursePage.sectionCurriculumHint")}
            required
          >
            <div className="flex flex-col gap-3">
              {weeks.map((w, wi) => {
                ...
                return (
                  <div
                    key={wi}
                    className={`rounded-xl border ${weekInvalid ? "border-destructive" : ""}`}
                  >
                    <div className="flex items-center gap-2.5 border-b bg-slate-50 px-3.5 py-2.5">
                      ...
                      <Input
                        ...
                        value={w.title}
                        onChange={(e) =>
                          setWeeks(upd(weeks, wi, { title: e.target.value }))
                        }
                      />
                      <button
                        type="button"
                    ...
                    <div className="flex flex-col gap-1.5 p-2.5">
                      {w.lessons.map((l, li) => {
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`grid size-7 shrink-0 place-items-center rounded-lg ${isLive ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-500"}`}
                            >
                              <Headset size={14} />
                            </span>
                            ...
                            <span
                              className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold ${isLive ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-500"}`}
                            >
                            ...
                            {isLive && (
                              <button
                                type="button"
                                onClick={() =>
                                  setEditor({ wi, li, draft: toLiveDraft(l) })
                                }
              ...
              {attempted && weeks.length === 0 && (
                <p className="text-[11px] text-destructive">
                  {t("NewCoursePage.validationNoWeeks")}
                </p>
              )}
        ...
        <aside className="flex flex-col gap-4 lg:sticky lg:top-6 lg:self-start">
          <div className="rounded-2xl border bg-white p-3.5">
            ...
            <div
              ...
              style={{
                background: `linear-gradient(135deg, oklch(0.94 0.04 145) 0%, oklch(0.88 0.07 145) 100%)`,
              }}
            />
            <div className="mt-3">
              {category && (
              ...
              <div className="mt-3 flex gap-3.5 text-xs text-muted-foreground">
                <span>
                  {t("NewCoursePage.previewWeeks", {
                    count: weeks.filter(
                      (w) =>
                        w.title.trim() || w.lessons.some((l) => l.title.trim())
                    ).length,
                  })}
                </span>
                <span>
                  {t("NewCoursePage.previewLessons", { count: lessonCount })}
                </span>
              </div>
            </div>
          </div>
...
): WeekDraft[] => weeks.map((w, idx) => (idx === wi ? { ...w, ...patch } : w));
