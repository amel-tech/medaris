...
`pnpm test` reports three projects, but only two of them run real tests: **299 tests across 22 suites, all in `tedrisat` and `teskilat`** (297 / 20 and 2 / 2). The third, `tedris-web`, executes `echo 'Tests not implemented'`, which Nx counts as a pass. **Frontend test coverage is zero.**
