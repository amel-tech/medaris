...
`pnpm test` reports three projects, but only two of them run real tests: **133 tests across 12 suites, all in `tedrisat` and `teskilat`** (131 / 10 and 2 / 2). The third, `tedris-web`, executes `echo 'Tests not implemented'`, which Nx counts as a pass. **Frontend test coverage is zero.** Migrating to Vitest and closing that gap is MDRS-20.
...
- **Swagger ships off, and tedrisat refuses to start rather than publish it by accident.** `SWAGGER_ENABLED=true` together with `NODE_ENV=production` throws in the config factory, before `listen()`, unless `SWAGGER_ALLOW_IN_PRODUCTION=true` is also set — publishing the schema also relaxes CSP and COOP on the Swagger pages. This bites locally too: `apps/tedrisat/Dockerfile` hardcodes `NODE_ENV=production`, so `docker compose up tedrisat` with an older `apps/tedrisat/.env` still carrying `SWAGGER_ENABLED=true` crash-loops with that message. `.env` is git-ignored and is not re-copied, so fix the file, not the template. `SWAGGER_ENABLED=true` also requires `KEYCLOAK_REDIRECT_URL`, which Swagger's OAuth2 redirect URL is built on.
