...
`pnpm test` reports three projects, but only two of them run real tests: **207 tests across 14 suites, all in `tedrisat` and `teskilat`** (205 / 12 and 2 / 2). The third, `tedris-web`, executes `echo 'Tests not implemented'`, which Nx counts as a pass. **Frontend test coverage is zero.**
...
Five of tedrisat's twelve suites are the `test/e2e/*.e2e.spec.ts` files: `apps/tedrisat/vitest.config.ts` matches them too, so **`pnpm test` needs a running Docker daemon** — those suites start a Testcontainers `postgres:17-alpine`. `pnpm --filter @medaris/tedrisat test:e2e` runs the same five under `apps/tedrisat/vitest.integration.config.ts` rather than adding coverage.
