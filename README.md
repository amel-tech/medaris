...
`pnpm test` reports three projects, but only two of them run real tests: **140 tests across 12 suites, all in `tedrisat` and `teskilat`** (138 / 10 and 2 / 2). The third, `tedris-web`, executes `echo 'Tests not implemented'`, which Nx counts as a pass. **Frontend test coverage is zero.** Migrating to Vitest and closing that gap is MDRS-20.
