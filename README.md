...
`pnpm test` reports three projects, but only two of them run real tests: **154 tests across 13 suites, all in `tedrisat` and `teskilat`** (152 / 11 and 2 / 2). The third, `tedris-web`, executes `echo 'Tests not implemented'`, which Nx counts as a pass. **Frontend test coverage is zero.** Migrating to Vitest and closing that gap is MDRS-20.

Five of tedrisat's eleven suites are the `test/e2e/*.e2e.spec.ts` files: `jest.config.json` matches them too, so **`pnpm test` needs a running Docker daemon** — those suites start a Testcontainers postgres. `pnpm --filter @medaris/tedrisat test:e2e` runs the same five under a separate config rather than adding coverage.
