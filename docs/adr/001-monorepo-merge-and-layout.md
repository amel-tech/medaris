...
- Root `eslint.config.mjs` = `nx.configs['flat/base']` + `@nx/enforce-module-boundaries` at `error` with `{ enforceBuildableLibDependency: true, allow: ['../../vitest.config', '../../vitest.integration.config'], depConstraints: <above> }` (the two `allow` entries were added by MDRS-20, each with its removal condition inline; MDRS-13 landed with `allow: []`); `linterOptions.reportUnusedDisableDirectives: "off"` (MDRS-12). It opens with R1's comment: *"ESLint is used ONLY for Nx module boundary enforcement. All other linting and formatting is handled by Biome."*
