...
| `.github/workflows/{tedrisat-api,teskilat-api,tedris-web,nizam-web,nazir-web,landing-web}.yaml` | a pinned `docker/setup-buildx-action` step — see *The first real run* below |
...
4. Push the 43 tags (MDRS-9 §7 step 2) before merging any release-please pull
   request — **done 2026-09-22**. All 43 tag commits were first checked with
   `git merge-base --is-ancestor <sha> origin/main` (43 ancestors, 0
   unreachable), then pushed; `gh api --paginate repos/amel-tech/medaris/tags
   --jq '.[].name' | wc -l` → `43`. No workflow fires on a tag push. The seven
   release-please PRs were regenerated by `workflow_dispatch` afterwards and
   now start from each component's own last release.
