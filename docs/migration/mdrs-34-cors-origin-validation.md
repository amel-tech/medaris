...
- `libs/common/src/config/cors.config.ts` is now four pure functions —
  `parseCsv`, `resolveAllowedOrigins`, `resolveAllowedMethods`,
  `buildCorsConfig` — each taking an explicit `NodeJS.ProcessEnv` that defaults
  to `process.env`. The exported constant `corsConfig` is gone; nothing outside
  `setupMiddleware.ts` referenced it.
  - A wildcard is passed as the bare string `"*"`, never inside an array.
  - Under `NODE_ENV=production` an unset, empty or wildcard-containing list
    throws at boot with a message naming `ALLOWED_ORIGINS`.
  - Outside production an unset or wildcard list still resolves to `"*"`, which
    is what running the web apps on localhost needs.
...
- `apps/tedrisat/test/unit/cors.config.spec.ts` — 22 cases over the parser.
...
**129 tests / 12 suites** (tedrisat 127/10, teskilat 2/2 — up from 106/11 by
this task's 23 cases), `build` 8, `lint` 16, `module-boundaries` 16, plus
