...
Since MDRS-86, `.github/workflows/nazir-web.yaml` passes them as
...
|---|---|---|
| `NAZIR_WEB_NEXT_PUBLIC_KEYCLOAK_ISSUER` | required | `NEXT_PUBLIC_KEYCLOAK_ISSUER` |
| `NAZIR_WEB_NEXT_PUBLIC_KEYCLOAK_CLIENT_ID` | required | `NEXT_PUBLIC_KEYCLOAK_CLIENT_ID` |
| `NAZIR_WEB_NEXT_PUBLIC_NEXTAUTH_URL` | required | `NEXT_PUBLIC_NEXTAUTH_URL` |
| `NAZIR_WEB_NEXT_PUBLIC_TEDRISAT_API_BASE_URL` | required | `NEXT_PUBLIC_TEDRISAT_API_BASE_URL` |
| `NAZIR_WEB_NEXT_PUBLIC_API_MOCKING` | optional — omitted when empty | `NEXT_PUBLIC_API_MOCKING` |
...
|---|---|
| Release created, tag `nazir-web-v1.4.0` | `1.4.0`, `latest`, `sha-<short>`, `stable` |
| Release created, tag `nazir-web-something-without-v` | `latest`, `sha-<short>`, `stable` — **no version tag** |
...
|---|---|---|---|---|
| development | the `development` one in the header | `latest` | *Manual path* or *Fan-out path* below, on `main` | `NAZIR_WEB_COOLIFY_WEBHOOK` |
| production | its twin in the `production` environment | `stable` | *Release path* below | `NAZIR_WEB_PROD_COOLIFY_WEBHOOK` |
