...
| Coolify webhook secret | `TEDRIS_WEB_COOLIFY_WEBHOOK` (repo secret — set 2026-09-16) |
...
1. **Webhook secret** — `TEDRIS_WEB_COOLIFY_WEBHOOK` was set on 2026-09-16; the
   deploy step's guard no longer fires.
2. **Image build in CI** — run `35536077688` (`.github/workflows/tedris-web.yaml`
...
**Still open (MDRS-87).** `TEDRIS_WEB_PROD_COOLIFY_WEBHOOK` is not set, and the Coolify
