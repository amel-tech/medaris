# Runbook — deploy Nizam Web

| | |
|---|---|
| Nx project | `nizam-web` |
| Source | `apps/nizam/` |
| Dockerfile | `apps/nizam/Dockerfile` (build context is the repo root) |
...
```yaml
type=match,pattern=nizam-web-v(.+),group=1
...
|---|---|
| Full release published, tag `nizam-web-v1.4.0` | `1.4.0`, `latest`, `sha-<short>`, `stable` |
| Full release published, tag `nizam-web-something-without-v` | `latest`, `sha-<short>`, `stable` — **no version tag** |
| Pre-release created, tag `nizam-web-v<semver>-rc.1` | `<semver>-rc.1`, `latest`, `sha-<short>` — **no `stable`**, so production is untouched and the deploy goes to development |
...
`type=match` strips the `nizam-web-v` prefix and keeps capture group 1, so the
release tag `nizam-web-v1.4.0` produces the image tag **`1.4.0`** — the
...
```yaml
if: ${{ github.event_name != 'release' || startsWith(github.event.release.tag_name, 'nizam-web-') }}
```

Read it as: *on a release, the tag must be ours; on anything else, run.* So a
release tagged `nizam-web-something-without-v` **runs the job but produces no
version tag** — only `latest` and `sha-…`. Always tag releases as `nizam-web-v<semver>`.
...
| production | its twin in the `production` environment | `stable` | *Release path* below, full releases only | `NIZAM_WEB_PROD_COOLIFY_WEBHOOK` |
...
* **Release path (production)** — merge the release-please PR for `nizam-web` (or
  create a GitHub release tagged `nizam-web-v<semver>` by hand). The workflow
...
gh api -H "Accept: application/vnd.github+json" \
  "/orgs/amel-tech/packages/container/medaris-nizam-web/versions" \
...
(Actions → **Nizam Web** → the run → "Record pushed image").

You can always address an old build by its immutable digest:
`ghcr.io/amel-tech/medaris-nizam-web@sha256:<digest>`.
...
docker buildx imagetools create \
  --tag ghcr.io/amel-tech/medaris-nizam-web:latest \
  ghcr.io/amel-tech/medaris-nizam-web@sha256:<good-digest>

# Confirm the move landed.
docker buildx imagetools inspect ghcr.io/amel-tech/medaris-nizam-web:latest
...
```bash
docker run --rm -p 4001:4001 ghcr.io/amel-tech/medaris-nizam-web:<tag>
...
4. **Order of operations: create the repo secrets BEFORE merging.** The runner
   ...
   But `apps/nizam/env.ts` marks `KEYCLOAK_CLIENT_ID`, `KEYCLOAK_CLIENT_SECRET`,
