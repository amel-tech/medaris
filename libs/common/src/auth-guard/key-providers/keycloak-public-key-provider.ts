...
@Injectable()
export class KeycloakPublicKeyProvider
  implements IPublicKeyProvider, OnModuleInit
{
  ...
  private loadConfig(jwksUrl?: string): KeycloakConfig {
    ...
    return {
      ...
      notFoundCacheTtl: parseInt(
        process.env.KEYCLOAK_NOT_FOUND_CACHE_TTL || "120"
      ), // 2 minutes
    };
  }
  ...
  async onModuleInit() {
    try {
      ...
      console.error(
        "Failed to pre-load JWKS keys during module initialization:",
        error
      );
    }
  }
